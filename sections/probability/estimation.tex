\chapter{Parameter estimation}\label{ch:param-est}
...
\begin{theorem}[Cramér-Rao bound]
    For any unbiased estimator $\hat{\xi}$ its variance-covariance matrix $\mathrm{V}_\xi[\hat{\xi}]$ satisfies
...
\begin{proof}
    ...
    \begin{align*}
        \mathrm{V}_{p_\xi}[A] &= \mathrm{E}_{p_\xi}[(c_i\hat{\xi}^{(i)}-c_i\xi^i)(c_j\hat{\xi}^{(j)}-c_j\xi^j)]
        \\
        &=\mathrm{E}_{p_\xi}[c_i(\hat{\xi}^{(i)}-\xi^i)(\hat{\xi}^{(j)}-\xi^j)c_j]
        \\
        &=c_i v_\xi^{ij} c_j
    \end{align*}
