...
\begin{equation}
    g_{ij}^{(S)}=S[\,\cdot\parallel\partial_i\partial_j\,]=\left.\partial_i\partial_j\sum_{x\in\mathcal{X}}p(x)\ln\left(\frac{p(x)}{q(x;\xi)}\right)\right\rvert_{q=p}
\end{equation}
for a fixed $p$ and $\partial_i=\frac{\partial}{\partial\xi^i}$. So we have
\begin{equation}
