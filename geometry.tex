...
\begin{equation}
    d(\mathbf{X},\mathbf{Y})=\lVert\mathbf{X}-\mathbf{Y}\rVert=\sqrt{\langle\mathbf{X}-\mathbf{Y},\mathbf{X}-\mathbf{Y}\rangle} \quad\forall\mathbf{X},\mathbf{Y}\in\mathcal{H}
\end{equation}
...
\end{equation}
so that the real part of the inner product also defines a metric tensor on the Hilbert space as explained in \cref{ch:square-dist}, endowing it with a Riemannian structure.

We thus have that the space of state vectors $\mathcal{H}_0$ is also a complex manifold with the Riemannian structure
\begin{equation}
    g_{\ket{\psi}}(\ket{d\psi_1},\ket{d\psi_2})=\mathrm{Re}\left[\braket{d\psi_1|d\psi_2}\right]
\end{equation}
This metric tensor is intrinsically derived from the structure of the Hilbert space and so it is invariant under unitary transformations. The set $U(N)$ of unitary transformations of $\mathbb{C}^N$ is a Lie group and so the Lie algebra of its left-invariant vector fields are the killing vector fields of the Hilbert space. The integral curves of these vector fields also preserve the norm of the points in the Hilbert space, and it can be shown that in each point their tangent vectors form a real $(2N-1)$-dimensional linear subspace of the tangent space. From this follows that the integral curves of the killing vector fields mesh to form a foliation of the realified Hilbert space where each leaf is the set of state vectors with a fixed norm and is thus isomorphic to $S^{2N-1}$. Then each leaf is a $(2N-1)$-dimensional submanifold of the realified Hilbert space and inherits a Riemannian structure.

\subsection{Fiber bundle structure of the Hilbert space} \label{sec:fibr-bund}
...
\begin{equation} \label{eq:tgt-operator}
...
Now we may ask ourselves if there is a natural way to induce a metric on $\mathbf{P}\mathcal{H}$ from the one of $\mathcal{H}$. Such an intrinsic metric would lead to a natural notion of distance between quantum states. To do this we will start by investigating the relationship between $\mathcal{H}$ and $\mathbf{P}\mathcal{H}$ as complex manifolds.
...
\subsection{The Fubini-Study metric}\label{ch:fubini-study}
...
Given a fiber bundle $X$ with base space $B$, the projection map $\pi:X\to B$ allows us to locally project any smooth curve on the fiber bundle to a smooth curve on the base space. From this follows that we have a natural map
\begin{equation}
...
At every point $x\in X$, we can choose a horizontal subspace $O_x$, which is complementary to the vertical subspace, such that
\begin{equation}
    T_xX=V_x\oplus O_x
\end{equation}
In general, there is no intrinsic way to choose a horizontal subspace at every point. However, if $X$ is also a Riemannian manifold, we can define the horizontal subspaces as the orthogonal complements of the vertical ones
\begin{equation}
    O_x\coloneqq V_x^\perp
\end{equation} 
and define horizontal vector fields in analogy with the vertical ones. This allows us to have a well-defined notion of vertical motion, tangent to the fibers, and horizontal motion, orthogonal to the fibers.

Intuitively, we can lift any curve in the base space to the fiber bundle by starting from a point on the fiber and then moving orthogonally to it. To precisely define this concept, let us consider the restriction to $O_x$ of the push-forward at every point
\begin{equation}
    d\pi_x'\equiv d\pi_x\big|_{O_x}:O_x\subseteq T_xX\to T_{\pi(x)}B
\end{equation}
It can be shown that the smoothness of the metric tensor field implies that $d\pi_x'$ defines a smooth map between the tangent bundles of the fiber bundle and the base space. From our previous considerations, we have
\begin{equation}
    \mathrm{ker}\,d\pi_x'=\{\mathbf{0}\}\qquad\forall x\in X
\end{equation}
which means that $d\pi_x'$ defines an isomorphism between $O_x$ and $T_{\pi(x)}B$ at every point. We can then uniquely define the \emph{horizontal lift} $v_l(x)\in T_xX$ of any vector field $v(y)\in T_yB$ on the base space as
\begin{equation}
    v_l(x)=d\pi_{y}'^{-1}(v(y))\, , \quad y=\pi(x) \qquad\forall x\in X
\end{equation}
This is the unique horizontal vector field on the fiber bundle that projects to $v(y)$.

We can now try to induce a metric tensor field on the base space by identifying its action on vector fields with the action of the metric tensor of the fiber bundle on the horizontal lift of the vector fields of the base space. Specifically, for given $dy_1,dy_2\in T_yB$, we have
\begin{equation}
    g'_y(dy_1,dy_2)=g_x(d\pi_{y}'^{-1}(dy_1),d\pi_{y}'^{-1}(dy_1))
\end{equation}
where $y=\pi(x)$. This is possible only if the scalar field resulting from the inner product of the horizontal lifts is constant on the fibers. In this case, we can push forward the scalar field to the base space and define the action of the induced metric tensor field on the original vector fields. Requiring the inner product of the horizontal lifts to be constant on the fibers is equivalent to requiring it to be constant under the action of the structure group $G$. Since horizontal lifts are invariant under the action of $G$, the only requirement is that the metric tensor field must be invariant, i.e., the left-invariant vector fields must also be killing vector fields.

We have thus shown that we can induce a metric tensor field on the base space of a Riemannian principal fiber bundle if the elements of the structure group are isometries of the fiber bundle. As we have seen in \cref{sec:geometry-HS}, the set of isometries of $\mathcal{H}_0=\mathbb{C}^N_0$ is the unitary group $U(N)$, while its structure group is the abelian group $\mathbf{C}_0$. However, not every element of the structure group is an isometry, since
\begin{equation}
    \mathbf{C}_0=\mathbb{R}^+\times U(1)\qquad c=\lVert c\rVert\cdot e^{i\theta}\leftrightarrow(\lVert c\rVert,e^{i\theta})
\end{equation}
To proceed, we must first fibrate with $\mathbb{R}^+$ and induce a metric on the base of this fibration. This can be easily done by defining the embedding of the projection as the normalized state vectors
\begin{equation}
...
\end{equation}
This defines the base space as one of the leaves of the foliation induced by $U(N)$ on $\mathcal{H}_0$
\begin{equation}
    S^{2N-1}=\mathbb{C}^N_0/\mathbb{R}^+
\end{equation}
as described in \cref{sec:fibr-bund}. The base space inherits a Riemannian structure such that
\begin{equation}
    g_{\ket{\psi}}(\ket{d\psi_1},\ket{d\psi_2})=\mathrm{Re}\left[\braket{d\psi_1|d\psi_2}\right]
\end{equation}
for any normalized state vector $\ket{\psi}$. Considering the restriction of the projection map of \cref{eq:proj-map-PH} on the set of normalized state vectors, we can repeat the same arguments as in \cref{sec:fibr-bund} and show that $S^{2N-1}$ is a Riemannian principal fiber bundle with structure group $U(1)$ and base space
\begin{equation} \label{eq:u-fibr}
    \mathbf{\mathbb{C}P}^n=S^{2N-1}/U(1)
\end{equation}
where $n=N-1$. The structure group is then a one-parameter subgroup of the isometry group, so we can induce a metric tensor field on $\mathbf{\mathbb{C}P}^n$ as described in this section. This metric for $\mathbf{\mathbb{C}P}^n$ is known as the Fubini-Study metric $g_{FS}$ and is intrinsically defined in terms of the structure of the Hilbert space only.

\subsubsection{The pullback of the FS metric on state vectors
}
We derive an explicit expression for the pullback of the Fubini-Study metric on $\mathcal{H}_0$. For any normalized state vector $\ket{\psi}$, we have
\begin{equation}
    g_{FS}(\ket{d\psi_1},\ket{d\psi_2})=g(d\pi_\rho'^{-1}(d\rho_1),d\pi_\rho'^{-1}(d\rho_2))
\end{equation}
where $\rho=\pi(\ket{\psi})$ and $d\rho_i=d\pi_{\ket{\psi}}(\ket{d\psi_i})$. We notice that the linear map
\begin{equation}
    d\pi_\rho'^{-1}\circ d\pi_{\ket{\psi}}:T_{\ket{\psi}}\mathcal{H}\to O_{\ket{\psi}}
\end{equation}
acts as the identity on $O_{\ket{\psi}}$ and has kernel $V_{\ket{\psi}}$, the same as $d\pi_{\ket{\psi}}$. Thus, we can recognize it as the orthogonal projector on $O_{\ket{\psi}}$. Since the motion along the fiber of $\ket{\psi}$ is given by $c\ket{\psi}$ for some complex number $c$, we have
\begin{equation}
    V_{\ket{\psi}}=\{c\ket{\psi}\quad \forall c\in\mathbb{C}\}
\end{equation}
and the projector to the vertical space is $\rho$. The projector to the horizontal space is then $(\mathbb{I}-\rho)$, so that 
\begin{equation}
...
\end{align}
Finally, for any state vector $\ket{\psi}\in\mathcal{H}_0$ and two tangent vectors $\ket{d\psi_1},\ket{d\psi_2}\in T_{\ket{\psi}}\mathcal{H}$, we have
\begin{equation}
...
\subsubsection{The FS metric on pure density operators}
We derive an explicit expression for the Fubini-Study metric on $\mathbf{\mathbb{C}P}^n$. As we have seen, the first fibration projects the state vectors to the normalized state vectors, which constitute a leaf of the foliation of submanifolds of $\mathcal{H}_0$ that are meshed by the integral curves of the left-invariant vector fields of $U(N)$. Then, at any point of $S^{2N-1}$, the tangent space is isomorphic to the Lie algebra of $U(N)$, which consists of anti-hermitian operators. This is because the action of any 1-parameter subgroup of $U(N)$ on a normalized state vector $\ket{\psi}$ can always be locally expressed as
\begin{equation}
    U(\theta)\ket{\psi}=e^{iA\theta}\ket{\psi}
\end{equation}
for some hermitian operator $A$. Thus, for any tangent vector $\ket{d\psi}\in T_{\ket{\psi}}S^{2N-1}$, there must exist a unique hermitian operator $A$ such that
\begin{equation}
    \ket{d\psi}=\left.\frac{d}{d\theta}\left(e^{iA\theta}\ket{\psi}\right)\right\rvert_{\theta=0} = iA\ket{\psi}
\end{equation}
The push forward of this vector is then given by
\begin{align}
    d\rho&=\left.\frac{d}{d\theta}\left(e^{iA\theta}\rho e^{-iA\theta}\right)\right\rvert_{\theta=0}=(iA\rho-\rho iA) \notag
    \\
    &=i[A,\rho]
\end{align}
We can express the pullback of the Fubini-Study metric on $S^{2N-1}$ in terms of the push forward of the tangent vectors
\begin{align}
...
\end{align}
Thus, the Fubini-Study metric on $\mathbf{\mathbb{C}P}^n$ is given by
\begin{equation}
