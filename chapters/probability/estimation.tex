...
In this section, we will use the Riemannian structure of the Fisher metric to derive certain results of probability theory and parameter estimation. First, we will define parameter estimation and their variance-covariance matrix. Next, we will show that the Fisher metric links the variance of random variables with the differential of their expectation value. Lastly, we will these results to state and prove the Cramér-Rao bound. The treatment follows the approach of \cite{amariMethodsInformationGeometry2007}.
