...
In this section, we discuss the differential geometrical structure of the complex projective space. We start by showing that both the Hilbert space and its complex projective space are complex manifolds, and we recognize that the inner product of the Hilbert space induces an intrinsic metric on itself. We show that the Hilbert space is a fiber bundle with the complex projective space as base space. Finally, through this structure, we induce an intrinsic metric to the complex projective space, the Fubini-Study metric. The treatment is based on \cite{ercolessiEquationsMotionCanonical2010}, \cite{contrerasGeometryMixedStates2016} and \cite{bengtssonGeometryQuantumStates2017} in general, and on \cite{schutzGeometricalMethodsMathematical1980} for the theory of fiber bundles and Lie groups; more details and different prospectives can be found in these references.

\subsection{Geometry of the Hilbert space} \label{sec:geometry-HS}
Complex manifolds are defined in analogy with real ones with the requirement of being locally isomorphic to $\mathbb{C}^N$ for some $N$ and with holomorphic transition functions between charts \cite{chernComplexManifoldsPotential1995}; the tangent space at each point is thus also isomorphic to $\mathbb{C}^N$. Every $N$-dimensional complex manifold is also a $2N$-dimensional real manifold where every complex coordinate basis $\{e_1,\dots,e_N\}$ corresponds to a real coordinate basis $\{e_1,\dots,e_N,\,ie_1,\dots,ie_N\}$, this manifold is called the \emph{realification} of the original complex one.
...
\end{equation}
where from the linearity of differentiation follows that $d\rho_\theta$ is a well-defined linear operator on $\mathcal{H}$.
...
\end{equation} 
and horizontal vector fields are defined in analogy with the vertical ones. We thus have a well-defined notion of vertical motion, tangent to the fibers, and of horizontal motion, orthogonal to the fibers.

Intuitively, we can now lift any curve in the base space to the fiber bundle by starting from a point on the fiber and then moving orthogonally to it. To precisely define this concept, let us consider the restriction to $O_x$ of the push-forward at every point
\begin{equation}
...
We can now try to induce a metric tensor field on the base space by identifying its action on vector fields with the action of the metric tensor of the fiber bundle on the horizontal lift of the vector fields of the base space, i.e., given $dy_1,dy_2\in T_yB$
\begin{equation}
    g'_y(dy_1,dy_2)=g_x(d\pi_{y}'^{-1}(dy_1),d\pi_{y}'^{-1}(dy_1))
\end{equation}
where $y=\pi(x)$. This is possible only if the scalar field resulting from the inner product of the horizontal lifts is constant on the fibers; only in this case, we can then push forward the scalar field to the base space and thus define the action of the induced metric tensor field on the original vector fields. Requiring the inner product of the horizontal lifts to be constant on the fibers is equivalent to requiring it to be constant under the action of the structure group $G$. By definition, horizontal lifts are invariant under the action of $G$, so the only requirement is that the metric tensor field must be invariant, i.e., the left-invariant vector fields must also be killing vector fields.
...
\end{equation}
where $\rho=\pi(\ket{\psi})$ and $d\rho_i=d\pi_{\ket{\psi}}(\ket{d\psi_i})$. We first notice that the linear map
\begin{equation}
    d\pi_\rho'^{-1}\circ d\pi_{\ket{\psi}}:T_{\ket{\psi}}\mathcal{H}\to O_{\ket{\psi}}
\end{equation}
must act as the identity on $O_{\ket{\psi}}$ and must have kernel $V_{\ket{\psi}}$, the same as $d\pi_{\ket{\psi}}$, so that we can recognize it as the orthogonal projector on $O_{\ket{\psi}}$. Since for every $\ket{\psi}$, the motion along its fiber is given by $c\ket{\psi}$ for some complex number $c$, then
\begin{equation}
    V_{\ket{\psi}}=\{c\ket{\psi}\quad \forall c\in\mathbb{C}\}
\end{equation}
and so the projector to the vertical space is $\rho$. The projector to the horizontal space is then $(\mathbb{I}-\rho)$ so that 
\begin{equation}
    d\pi_\rho'^{-1}\circ d\pi_{\ket{\psi}}(\ket{d\psi})=(\mathbb{I}-\rho)\ket{d\psi}=\ket{d\psi}-\braket{\psi | d\psi}\ket{\psi}
\end{equation}
and for normalized state vectors we have
\begin{align}
...
\end{align}
Finally, it can be similarly shown that for any state vector $\ket{\psi}\in\mathcal{H}_0$ and two tangent vectors $\ket{d\psi_1},\ket{d\psi_2}\in T_{\ket{\psi}}\mathcal{H}$ we have
\begin{equation}
...
\begin{align}
    ...
    &=i[A,\rho]
\end{align}
We can then express the pullback of the Fubini-Study metric on $S^{2N-1}$ in terms of the push forward of the tangent vectors
\begin{align}
...
\end{align}
and so we find that the Fubini-Study metric on $\mathbf{\mathbb{C}P}^n$ is given by
\begin{equation}
