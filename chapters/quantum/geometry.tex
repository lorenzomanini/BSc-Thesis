...
\end{equation}
must act as the identity on $O_{\ket{\psi}}$ and must have kernel $V_{\ket{\psi}}$, the same of $d\pi_{\ket{\psi}}$, so that we can recognize it as the orthogonal projector on $O_{\ket{\psi}}$. Since for every $\ket{\psi}$ the motion along its fiber is given by $c\ket{\psi}$ for some complex number $c$, then
\begin{equation}
