...
\begin{corollary}
    Any self-adjoint operator may be 
\end{corollary}
From the definition of a complete set of orthogonal projectors and 


Then, we may uniquely associate to any observable the self-adjoint operator
\begin{equation} \label{eq:observable-op}
    A=\sum_{i}\lambda_i P_i
\end{equation}
Finally, we may ask ourselves if the converse is true, i.e. if we may associate a unique PVM to any self-adjoint operator, expressing it as in \ref{eq:observable-op}. The following theorem ensures that this is the case.
