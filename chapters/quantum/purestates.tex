...
We have seen that quantum states are the 1-dim linear subspaces of $\mathcal{H}$, so we can identify them with orthogonal projectors. The only additional requirement is that the projected spaces must be 1-dim, i.e., they must be rank-1 orthogonal projectors. From \cref{eq:proj-decomp}, we know that for any rank-1 projector, there must exist a normalized vector $\ket{\tilde{e}}$ such that
\begin{equation}
