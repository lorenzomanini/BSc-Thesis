...
Given the postulates of \cref{sec:postulates} we may ask ourselves if there is a 1-1 correspondence between quantum states and state vectors. \Cref{pt:state-vect} ensures that every state vector corresponds to a quantum state and that for each quantum state, there is at least one state vector that corresponds to it. Though it might be that multiple state vectors correspond to the same quantum state, and as we shall see this is the case.

We start by requiring that two equivalent state vectors have the same probability distributions for any measurement. This means that for two equivalent state vectors $\ket{\psi}$, $\ket{\psi'}$
\begin{equation}
    \frac{\braket{\psi|\mathrm{M}_x^\dag\,\mathrm{M}_x|\psi}}{\braket{\psi|\psi}}=\frac{\braket{\psi'|\mathrm{M}_x^\dag\,\mathrm{M}_x|\psi'}}{\braket{\psi'|\psi'}} \qquad \forall x
\end{equation}
...
\begin{equation}\label{eq:equiv-state-vect}
    \ket{\psi'}=c\ket{\psi} \quad \forall c\in\mathbb{C}
\end{equation}
