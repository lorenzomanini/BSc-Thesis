...
The chosen inner product allows us to define a canonical isomorphism between $\mathcal{H}$ and its dual vector space $\mathcal{H}^*$ so that for every $\mathbf{Z}\in\mathcal{H}$ its dual functional is defined as
...
\end{align}
and, in particular, given a vector $\mathbf{Y}$ and a dual vector $\mathbf{X}^*$ their tensor product is the operator
...
\end{equation}
also, after the measurement, the state vector is projected onto the subspace of the outcome
\begin{equation}
    \ket{\psi'}=P_x \ket{\psi}
\end{equation}
...
\end{equation}
and so the space of quantum states is isomorphic to the set of 1-dim linear subspaces of $\mathcal{H}$, also called \emph{complex lines}. This set is known as the \emph{projective space} of $\mathcal{H}$ and is denoted as $\mathbf{P}\mathcal{H}$. When $\mathcal{H}=\mathbb{C}^N$, it is known as the $n$-dimensional complex projective space $\mathbf{\mathbb{C}P}^n$, where $n=N-1$. Then \cref{eq:quantum-state} leads to the intrinsic definition of a surjective \emph{projection map}
\begin{equation}
...
We have seen that quantum states are the 1-dim linear subspaces of $\mathcal{H}$, thus we should be able to identify them with orthogonal projectors. The only added requirement is that the projected spaces must be 1-dim, i.e. they must be rank-1 orthogonal projectors. From \cref{eq:proj-decomp}, we have that for any rank-1 projector there must exist a normalized vector $\ket{\tilde{e}}$ such that
\begin{equation}
    P_{\tilde{e}}\ket{\psi}=\braket{\tilde{e}|\psi}\ket{\tilde{e}}\quad\forall\ket{\psi}\in\mathcal{H}_0
\end{equation}
and so from \cref{eq:outer-prod} follows that
\begin{equation}
    P_{\tilde{e}}=\ket{\tilde{e}}\bra{\tilde{e}}
\end{equation} \label{eq:rank1-proj}
In the context of quantum mechanics, the set on rank-1 orthogonal projectors is known as the set of \emph{pure density operators} and we will note it with $\mathfrak{D}$.
...
\end{equation}
Finally, we can prove that $f$ is invertible so that we may identify quantum states with density matrices. The map is 1-1 since given any two state vectors $\ket{\psi}$ and $\ket{\phi}$ we have that
\begin{align*}
...
\end{align*}
Then, since any rank-1 orthogonal projector can be expressed as in \cref{eq:rank1-proj} we also have that $f$ is SU. We have thus proved that
\begin{equation}
    \mathbf{P}\mathcal{H}\sim\mathfrak{D}
\end{equation}
through the intrinsic map $f$, so we may identify quantum states with pure density operators and we have that the two projection maps $\pi$ and $\Pi$ are the same under this identification.
...
\end{equation}
where it can be shown that the expression on the right is independent of the choice of the orthonormal base. The trace also has the following cyclic property
\begin{equation}
    \mathrm{tr}(AB)=\mathrm{tr}(BA)
\end{equation}
so that for any number of operators $A_1,\dots,A_n$ we have that
\begin{equation}
...
\begin{postulate}[Unitary evolution]
    The Hilbert space of a closed system evolves only through unitary transformations. Coherently, the time evolution of any pure2 density operator $\rho(t)$ describing the quantum state of the system is given by
    \begin{equation}
...
\begin{equation}
    T_{\ket{\psi}}\mathcal{H}\sim\mathcal{H}\qquad\mathrm{with}\qquad\frac{\partial}{\partial\theta}\leftrightarrow\frac{\partial\ket{\psi(\theta)}}{\partial\theta}\coloneqq\ket{d\psi_\theta}
\end{equation}
...
This metric tensor is intrinsically derived from the structure of the Hilbert space and so it is also invariant under unitary transformations. Moreover, the set $U(N)$ of unitary transformations of $\mathbb{C}^N$ is a Lie group and thus the Lie algebra of its left-invariant vector fields are the killing vector fields of the Hilbert space. These vector fields also preserve the norm of the points in the Hilbert space and it can be shown that in each point they form a real $(2N-1)$-dimensional linear subspace of the tangent space. From this follows that the integral curves of the killing vector fields mesh to form a foliation of submanifolds of the realifed Hilbert space, i.e. spheres of vectors with the same norm.

\subsection{T}
We are now interested in the geometry of the space of quantum states. For start we have that, with a sound choice of coordinates, one can easily verify that $\mathbf{\mathbb{C}P}^N$ is also a complex manifold. \todo{proof}. Then we may ask ourselves if there is a natural way to induce a metric on it from the one of $\mathcal{H}$. Such an intrinsic metric would lead to a natural notion of distance between quantum states that we would then have to interpret.

Firstly it can be shown that the intrinsically defined projection map of \cref{eq:proj-map}
\begin{equation*}
    \pi:\mathcal{H}_0\to\mathfrak{D}\sim\mathbf{\mathbb{C}P}^N
\end{equation*}
is a smooth SU map between the two manifolds so that we may regard $\mathcal{H}_0$ as a fibre bundle with base space $\mathbf{\mathbb{C}P}^N$. Let us now recall the definition of a quantum state from \cref{eq:quantum-state}
\begin{equation}
    \left[\ket{\psi}\right]_\sim=\{c\ket{\psi}\,\forall c\ne0 \in\mathbb{C}\}
\end{equation}
where we recognize that the fibers of the fiber bundle are the orbits of the action of the abelian Lie group
\begin{equation}
    \mathbf{C}_0=\{c\cdot\mathbb{I}\mid\forall c\ne0\in\mathbb{C}\}
\end{equation}
on $\mathcal{H}_0$. We can thus write
\begin{equation}\label{eq:cpn-quotient}
    \mathbf{\mathbb{C}P}^N=\mathcal{H}_0/\mathbf{C}_0
\end{equation}
and $\mathcal{H}_0$ is a principle fibre bundle\cref. \todo{No cross-section?}
