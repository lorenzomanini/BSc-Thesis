...
\begin{equation}
    \mathfrak{M}_\rho:A\mapsto\left.\frac{d}{d\theta}\left(e^{iA\theta}\rho e^{-iA\theta}\right)\right\rvert_{\theta=0}=i[A,\rho]\eqqcolon d\rho_A\in T_\rho\mathfrak{D}
\end{equation}
...
We can then consider the restriction of $\mathfrak{M}_\rho$ to the observables with zero expectation value, denoting it as $\mathfrak{M}_\rho^{(0)}$. Through the matrix representation of operators we can easily compute the real dimensions of $\mathfrak{A}^\bot$ and of the kernel of $\mathfrak{M}_\rho^{(0)}$. These turn out to be respectively $(N^2-1)$ and $(N-1)^2$ \todo{proof?} so that the image of $\mathfrak{M}_\rho^{(0)}$ must have real dimension $(2N-2)$, equal to the dimension of the tangent space; this proves that $\mathfrak{M}_\rho^{(0)}$ is surjective. Then we can also show that $\mathfrak{M}_\rho^{(0)}$ maps $T_\rho\mathfrak{D}$ to itself since
\begin{align*}
...
\end{equation}
where $\mathfrak{A}^\bot_{C(\rho)}$ is the set of zero expectation value observables that commute with $\rho$. We thus have the unique decomposition
\begin{gather}
    A=A_{C(\rho)}+A^{(e)}_\rho\qquad\forall A\in\mathfrak{A}^\bot\notag
    \\
    \mathrm{with}\quad A_{C(\rho)}\in\mathfrak{A}^\bot_{C(\rho)}\quad\mathrm{and}\quad A^{(e)}_\rho\in T_\rho\mathfrak{D}
\end{gather}
...
\begin{align}
    ...
    \\
    &=\frac{1}{2}\,\mathrm{tr}\left(\rho\left\{A^{(e)}_\rho,\,B^{(e)}_\rho\right\}\right)=\llangle A^{(e)}_\rho,B^{(e)}_\rho \rrangle_\rho
\end{align}
so that we can define the map
\begin{equation}
    A\mapsto \tilde{A}_\rho\in T_\rho\mathbf{P}\mathcal{H}\qquad\forall A\in\mathfrak{A}^\bot,\,\rho\in\mathbf{P}\mathcal{H}
\end{equation}
where $A_\rho$ is the tangent vector whose exponential rapresenntation is $A^{(e)}_\rho$. Then we have also proved that
\begin{equation}
    \llangle A,B \rrangle_\rho=G_{QF}(\tilde{A}_\rho,\tilde{B}_\rho)
\end{equation}
