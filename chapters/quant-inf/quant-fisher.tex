...
\footnote{
    This shows that we can define $\mathbb{C}\mathbf{P}^n=\frac{U(N)}{U(1)\times U(N-1)}$. Through the same arguments, this expression can be easily generalized to mixed states, so that the space of rank-m density matrices is given by $\frac{U(N)}{U(m)\times U(N-m)}$. This is treated in \cite{contrerasGeometryMixedStates2016}.
}
